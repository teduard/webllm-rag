...
function TokenBar({ file }: { file: FileState }) {
  const pct      = Math.min(100, Math.round(
    (file.tokenCount / (file.tokenCount / (file.mode === "direct" ? 1 : 1))) * 100
  ));
  ...
  return (
    <div className={s.tokenBar}>
      ...
      <span className={s.tokenCount}>{file.tokenCount.toLocaleString()} tokens</span>
      <span className={s.modePill} style={{ color }}>
        {isDirect ? "● direct" : "● rag"}
      </span>
      <button className={s.newFile} onClick={() => {}}>× new file</button>
    </div>
  );
}

export function Chat({ file, messages, isThinking, onSend, onReset }: Props) {
    ...
    <div className={s.panel}>
      ...
      <div className={s.topBar}>
        <div className={s.fileInfo}>
          ...
          {/* <span
            ...
            title={file.mode === "direct"
              ? "File fits in context window - full content used"
              : "File exceeds context window - answering from retrieved excerpts"}
          >
            {file.mode === "direct" ? "● direct context" : "● rag retrieval"}
          </span> */}
        </div>
        <button className={s.newFileBtn} onClick={onReset}>Clear chat</button>
      </div>
      ...
      <div className={s.messages}>
        ...
        {messages.map((m, i) => (
          <div key={i} className={`${s.bubble} ${m.role === "user" ? s.user : s.assistant}`}>
            ...
            {m.role === "assistant" ? (
              <div className={s.md}>
                {m.text ? <ReactMarkdown>{m.text}</ReactMarkdown> : <span className={s.cursor} />}
              </div>
            ) : (
      ...
      <div className={s.inputRow}>
        <textarea
          ...
          rows={1}
        />
        <button className={s.sendBtn} onClick={submit} disabled={isThinking || !draft.trim()}>↵</button>
      </div>
    </div>
  );
}
