...
export function ModelPicker({ selectedId, status, onSelect, onLoad }: Props) {
  ...
  const pct = isLoading ? Math.round(status.progress * 100) : 0;
  const txt = isLoading ? status.text : "";
        ...
        <div className={s.grid}>
          {MODELS.map((m) => (
            ...
            >
              ...
              <span className={s.cardCtx}>
                context window: {m.contextWindow.toString()} tokens
              </span>
      ...
      <div className={s.section}>
        ...
        {isLoading ? (
          ...
          <button onClick={onLoad}>
            {isReady ? (
              <>
                <span>Reload</span> <ArrowPathIcon className="heroIcon" />
              </>
            ) : (
              <>
                <span>Load</span>
